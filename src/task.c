...
static void clean_up(uv_handle_t *handle);
...
static void run_udp_task(uv_loop_t *loop, query_task_t *task) {
    ...
    req->buf = uv_buf_init(task->msg, task->msg_len);
...
static void clean_up(uv_handle_t *handle) {
    query_task_t *task = get_task(handle);
...
static void on_read_tcp_response(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf) {
    if (nread < 0) {
        ...
        task->response_count += 1;
        task->cb(task, buf->base, nread, (uint64_t) ((uv_hrtime() - task->start_time) / 1e6));
        uv_read_stop(stream); // tcp query may only have one response.
        clean_up((uv_handle_t *) stream);
    }
